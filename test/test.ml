...
type schema_optional = { name : string; address : string option }
[@@deriving show, eq]
...
let decode_optional () =
  ...
  Alcotest.(
    check
      ...
      "has no name" (Error "Failed to decode field 'name': No value provided")
      (C.decode schema []));
  Alcotest.(
    check
      (result testable_schema_optional string)
...
let decode_complete_and_invalid_input () =
  ...
  Alcotest.(
    check testable_decode_result "has one error"
      (Error
...
let decode_complete_and_valid_input () =
  ...
  let actual = C.decode Schema.user_schema input in
  let expected =
    Schema.user Schema.Male "walter" "test@example.com" (2020, 12, 01)
      "Switzerland" 3 (Some "hello") true
  in
